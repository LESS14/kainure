...
const STANDARD_SIGNATURES = {
    'OnDialogResponse': 'iiiis',      // playerid, dialogid, response, listitem, inputtext
    ...
    'OnUnoccupiedVehicleUpdate': 'iiifff',
};
...
class Kainure_Core extends Event_Emitter {
    constructor() {
        ...
        for (const [event, sig] of Object.entries(STANDARD_SIGNATURES)) {
            this.signatures.set(event, sig);
        }
    }
    ...
    Public(event_name, callback) {
        ...
        if (!this.signatures.has(event_name)) {
...
globalThis.Ref = (initial_value = 0, is_float_ref = false) => {
    const obj = {
        value: initial_value,
        __kainure_ref: true,
        get $() { return { __kainure_ptr: true, parent: obj }; },
        valueOf() { return this.value; },
        toString() { return String(this.value); }
    };
    if (is_float_ref) {
        obj.__kainure_float_ref = true;
    }
    return obj;
};
...
globalThis.Call_Public = new Proxy({}, {
    get(_, prop) {
        return (...args) => {
            if (!kainure.call_public) throw new Error('"call_public" not initialized by C++ yet.');
...
globalThis.Include_Storage = (folder_name) => {
    ...
    const include_storage_path = path.resolve(process.cwd(), 'Kainure', 'includes_storage', safe_name);
    if (!fs.existsSync(include_storage_path)) fs.mkdirSync(include_storage_path, { recursive: true });
...
globalThis.Kainure_Emit_Event = (name, ...args) => {
    ...
    let last_result;
    for (const listener of listeners) last_result = listener(...args);
    return last_result;
};

globalThis.Kainure_Has_Listeners = (name) => kainure.listenerCount(name) > 0;
globalThis.Kainure_Get_Signature = (name) => kainure.signatures.get(name) || "";
